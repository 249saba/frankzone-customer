...
const ProductItemCard = ({ items, handleClick }: any) => {
  return (
    <CustomCard
      onClick={() => handleClick(items?.id)}
      styleClass="shadow-sm h-full cursor-pointer w-full "
    >
      <div className="relative ">
        <LazyImage
          src={
            items?.DefaultVariant?.cover_image_url
              ? 
                items?.DefaultVariant?.cover_image_url
              : HomeBg
          }
          ...
          className="object-cover  h-[260px] rounded-t-xl w-full "
        />
        <p
