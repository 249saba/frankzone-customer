...
const ProductDetail = () => {
  ...
  const getDeals = (productId: any) => {
    backendCall({
      url: `/api/user/home/similar_products/${productId}`,
      method: "GET",
    }).then((res: any) => {
      if (res && !res.error) {
  ...
  const getProductDetail = () => {
    backendCall({
      url: `/api/user/deal/product/${params?.productid}`,
      method: "GET",
    }).then((res: any) => {
      if (res && !res.error) {
        setProductDetail(res?.data);
        getDeals(res?.data.id);
  ...
  const handleAddToCart = () => {
    ...
    const obj = {
      ...
      vendor_deal_id: productDetail?.DefaultVariant?.vendor_deal_id,
    };
    backendCall({
      ...
      if (res && !res.error) {
        ...
        navigate({
          ...
          search: `?vendor=${vendor}&vendorDetail=${vendorDetail}`,
        });
      } else {
  ...
  const handleAttributes = (
    ...
    backendCall({
      url: `/api/user/home/product/${productDetail?.id}/get_attributes`,
      method: "POST",
  ...
  const handleProductVariants = () => {
    backendCall({
      url: `/api/user/home/product/${productDetail?.id}/product_variant`,
      method: "POST",
      data: {
        attributes: dynamicAttributes,
        deal_product_id: params?.id,
      },
    }).then((res: any) => {
      if (res && !res.error) {
        dynamicAttributes = [];
    ...
    <>
      ...
      <div className="flex md:flex-col sm:flex-col gap-8  items-start">
        ...
        <div className="flex flex-col items-start text-left gap-3 w-[72%] sm:w-[100%]">
            ...
            <div className="flex justify-between sm:flex-col sm:gap-6">
              <div className="flex items-center gap-3">
                <div
                  ...
                  onClick={() =>
                    navigate(
                      `/vendors/vendorDetail/${productDetail?.vendor_id}`
                    )
                  }
                >
                  <LazyImage
          ...
          <div className="flex items-end">
            <h3 className="text-[#F44336] font-medium ">
              €{singleVariant?.discounted_price}
            </h3>
            <h4 className="text-gray-900 pl-3 line-through mb-2">
              €{singleVariant?.price}
            </h4>
          </div>
        ...
        <div className="  text-center  w-[100%]  ">
          ...
          productDetail?.ProductAttributes.map((attributes: any) => {
            return (
              <>
                <h6 className="text-black-900 font-medium">
                  {attributes?.VendorAttribute?.name}
                </h6>
                <div className="flex items-center gap-3">
                  {attributes?.ProductOptions &&
                    attributes?.ProductOptions.map(
                      (option: any, index: any) => (
                        <p className="bg-green-800 bg-opacity-10 text-green-700 rounded-full py-1 px-4">
                          {option?.AttributeOption?.name}
                        </p>
                      )
                    )}
          ...
          {productDetail?.Vendor?.ActivePlan?.Plan?.can_sell_deals ? (
            <>
              <hr className="border border-gray-800 w-full mt-4" />
      ...
      <div className="bg-[#757ff25c] h-20 w-full flex justify-start items-center mt-8">
        <div
          ...
          onClick={() =>
            navigate({
              search: `?dealid=${params.dealid}&productid=${params.productid}&vendor=${vendor}&vendorDetail=${vendorDetail}`,
              pathname: `/vendors/reviews/${productDetail?.vendor_id}`,
            })
          }
        >
          <div className="flex justify-start items-center ">
            ...
            />
            <p className="ml-4 text-[#7580F2] text-xl">Real All</p>
          </div>
          <LazyImage
            src={rightArrow}
            alt=""
                ...
                >
                  <ProductItemCard
                    ...
                    handleClick={(id: any) =>
                      navigate({
                        search: `?vendor=${vendor}&vendorDetail=${vendorDetail}`,
                        pathname: `/vendors/${vendor}/vendorDetail/${vendorDetail}/Item/${id}`,
                      })
                    }
                  />
                </Slide>
